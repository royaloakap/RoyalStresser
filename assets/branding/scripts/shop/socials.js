        ...
        if (result.isConfirmed) {
            $.post('/api/payments/buy', { plan_name: type }, function (data) {
                ...
                if (response.status == "success") {
                    toastr['success']('Payment successful, Reconnect You for update !', 'Store', { "toastClass": "toast-dark" });
                } else if (response.status == "error") {
                    toastr['warning'](response.message, 'Store', { "toastClass": "toast-dark" });
                } else {
                    toastr['error']('Failed to purchase plans', 'Error', { "toastClass": "toast-dark" });
                }
            }).fail(handleErrors);
        };
    });
...
(function () {
            ...
            <div class="card-body">
                ...
                <div class="text-center">
                    <div class="d-flex justify-content-center">
                        ...
                        <sub class="h6 pricing-duration mt-auto mb-2 text-muted fw-normal">/${plan.expiry}d</sub>
                    </div>
                </div>

                <ul class="ps-3 my-4 pt-2">
                    <li class="mb-2">${plan.duration} Seconds</li>
                    <li class="mb-2">${plan.concurrents} Concs</li>
                    <li class="mb-2">Unlimited Test</li>
                    <li class="mb-0">VIP ${plan.vip}</li>
                    <li class="mb-0">API ${plan.api}</li>
                </ul>

                <button
                    class="btn btn-primary btn-md waves-effect waves-light" type="submit" onclick="buyPlan('${plan.name}')">socials</button>
            </div>
        </div>
    `;
    ...
    function loadPlans(data) {
        if (data.status === "success") {
            // Trier les plans par ordre alphabétique sur le champ "name"
            data.plans.sort((a, b) => a.name.localeCompare(b.name));

            // Effacer les conteneurs pour éviter les doublons lors du rechargement
            document.querySelector('.all-plans-cards').innerHTML = "";
            document.querySelector('.all-plans-cards2').innerHTML = "";

            // Afficher les plans triés
