...
func Send(a *floods.Attack) {
	...
	for _, api := range Apis {
		...
		// Nettoyer la méthode et l'URL
		method = strings.TrimSpace(method)
		url := strings.TrimSpace(terms.Replace(api.URL))
		url = strings.TrimSpace(strings.ReplaceAll(url, "$method", method))

		// Affichez l'URL pour débogage
