...
func (c *Client) NewPayment(amount int, fiat, title, email, gateway, coupon string, req *http.Request) *Payment {
	return &Payment{
		PaymentReq: &PaymentReq{
			Title:         "Royal Projets Payment",
			Gateway:       gateway,
			Value:         float64(amount),
			Currency:      fiat,
			CouponCode:    coupon,
			Confirmations: 2,
			Webhook:       "https://discord.com/api/webhooks/1331465887886217277/tzWypkERyLY6mumkH_xqmHtFqYzinWJAhcCVC3qxLC8YViloBvujcHagfz00p2bMQ4G3",
